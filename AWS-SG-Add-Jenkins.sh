...
echo '------------------Add Inbound rule to the created security group jenkins  Machine '
...
aws ec2 authorize-security-group-ingress --group-name $GROUP_NAME --protocol tcp --port 8080 --cidr 0.0.0.0/0 --output json
# Docker Machine 
aws ec2 authorize-security-group-ingress --group-name $GROUP_NAME --protocol tcp --port 2376 --cidr 0.0.0.0/0 --output json

# 50000 for jenkins slave 
aws ec2 authorize-security-group-ingress --group-name $GROUP_NAME --protocol tcp --port 50000 --cidr 0.0.0.0/0 --output json
